...
Some process will use temporary tables in the database file as intermediaries in the process of creating the final files and/or tables. These tables are named `temp_XXXXXXXXXXXXXX` where the X's are a large integer, which is generated by hashing the timestamp of when the table was created. If you name tables in such a manner, their is a chance (though very unlikely) that they may be deleted. Also, while all temporary tables are deleted after being used, if a process fails, there is a chance that the table remains; these tables can be deleted with no consequence.
